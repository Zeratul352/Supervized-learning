...
import pandas as pd
import numpy as np
from src.naive_bayes_classifier import (read_csv, train_naive_bayes_classifier, evaluate_model)
...
def test_train_naive_bayes_classifier():
    """
    Test if train_naive_bayes_classifier runs successfully.
    """
    X = np.array([[1, 2], [3, 4], [5, 6]])
    y = np.array([0, 1, 1])

    model = train_naive_bayes_classifier(X, y)
